...
// Resumo do mês (total vendido + comissão + total dia + média diária)
  ...
  try {
    const result = await pool.query(
      `SELECT
         ...
         SUM(CASE WHEN v.data_venda = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date THEN v.valor_total ELSE 0 END) AS total_dia,
